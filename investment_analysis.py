"""
Comprehensive Investment Analysis Module for Renewable Energy Companies
Includes Technical, Fundamental, Risk Analysis, and Portfolio Optimization
"""

import sys
import traceback
import logging
from datetime import datetime, timedelta
import warnings
warnings.filterwarnings('ignore')

# Set up logging to capture all errors
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(sys.stdout),
        logging.FileHandler('investment_analysis.log')
    ]
)

try:
    import pandas as pd
    import numpy as np
    import yfinance as yf
    from scipy import stats
    from scipy.optimize import minimize
    import matplotlib.pyplot as plt
    import seaborn as sns
except ImportError as e:
    print(f"ERROR: Failed to import required modules: {e}")
    print("Please install required packages: pip install pandas numpy yfinance scipy matplotlib seaborn")
    sys.exit(1)

try:
    from config import *
    from database import DatabaseManager
except ImportError as e:
    print(f"ERROR: Failed to import config or database modules: {e}")
    print("Please ensure config.py and database.py are in the same directory")
    sys.exit(1)

class InvestmentAnalyzer:
    """Comprehensive investment analysis for renewable energy companies"""
    
    def __init__(self):
        """Initialize the analyzer"""
        try:
            self.db_manager = DatabaseManager()
            self.companies = {
                'Orsted': 'ORSTED.CO',
                'Vestas': 'VWS.CO',
                'NextEra Energy': 'NEE',
                'Iberdrola': 'IBE.MC',
                'Enel': 'ENEL.MI',
                'Brookfield Renewable': 'BEP',
                'EDP Renovaveis': 'EDPR.LS',
                'Siemens Gamesa': 'SGRE.MC',
                'Plug Power': 'PLUG',
                'First Solar': 'FSLR'
            }
            print("InvestmentAnalyzer initialized successfully")
        except Exception as e:
            print(f"ERROR: Failed to initialize InvestmentAnalyzer: {e}")
            logging.error(f"Initialization error: {traceback.format_exc()}")
            raise
        
    def calculate_returns(self, prices):
        """Calculate daily and cumulative returns"""
        try:
            if prices is None or prices.empty:
                print("ERROR: No price data provided for returns calculation")
                return None, None
            
            # Handle both DataFrame and Series inputs
            if hasattr(prices, 'columns'):
                # DataFrame input
                if 'Close' not in prices.columns:
                    print("ERROR: 'Close' column not found in price data")
                    return None, None
                price_series = prices['Close']
            else:
                # Series input - assume it's already the price series
                price_series = prices
            
            returns = price_series.pct_change().dropna()
            cumulative_returns = (1 + returns).cumprod()
            return returns, cumulative_returns
        except Exception as e:
            print(f"ERROR: Failed to calculate returns: {e}")
            logging.error(f"Returns calculation error: {traceback.format_exc()}")
            return None, None
    
    def calculate_volatility(self, returns, window=252):
        """Calculate rolling volatility"""
        try:
            if returns is None or returns.empty:
                print("ERROR: No returns data provided for volatility calculation")
                return None
            
            return returns.rolling(window=window).std() * np.sqrt(252)
        except Exception as e:
            print(f"ERROR: Failed to calculate volatility: {e}")
            logging.error(f"Volatility calculation error: {traceback.format_exc()}")
            return None
    
    def calculate_beta(self, stock_returns, market_returns, window=252):
        """Calculate rolling beta"""
        try:
            if stock_returns is None or market_returns is None:
                print("ERROR: Missing returns data for beta calculation")
                return None
            
            beta = stock_returns.rolling(window=window).cov(market_returns) / market_returns.rolling(window=window).var()
            return beta
        except Exception as e:
            print(f"ERROR: Failed to calculate beta: {e}")
            logging.error(f"Beta calculation error: {traceback.format_exc()}")
            return None
    
    def calculate_sharpe_ratio(self, returns, risk_free_rate=0.02, window=252):
        """Calculate Sharpe ratio"""
        try:
            if returns is None or returns.empty:
                print("ERROR: No returns data provided for Sharpe ratio calculation")
                return None
            
            excess_returns = returns - risk_free_rate/252
            sharpe = excess_returns.rolling(window=window).mean() / returns.rolling(window=window).std()
            return sharpe * np.sqrt(252)
        except Exception as e:
            print(f"ERROR: Failed to calculate Sharpe ratio: {e}")
            logging.error(f"Sharpe ratio calculation error: {traceback.format_exc()}")
            return None
    
    def calculate_max_drawdown(self, cumulative_returns):
        """Calculate maximum drawdown"""
        try:
            if cumulative_returns is None or cumulative_returns.empty:
                print("ERROR: No cumulative returns data provided for drawdown calculation")
                return None
            
            rolling_max = cumulative_returns.expanding().max()
            drawdown = (cumulative_returns - rolling_max) / rolling_max
            max_drawdown = drawdown.min()
            return max_drawdown
        except Exception as e:
            print(f"ERROR: Failed to calculate maximum drawdown: {e}")
            logging.error(f"Max drawdown calculation error: {traceback.format_exc()}")
            return None
    
    def calculate_var(self, returns, confidence_level=0.05):
        """Calculate Value at Risk"""
        try:
            if returns is None or returns.empty:
                print("ERROR: No returns data provided for VaR calculation")
                return None
            
            return np.percentile(returns, confidence_level * 100)
        except Exception as e:
            print(f"ERROR: Failed to calculate VaR: {e}")
            logging.error(f"VaR calculation error: {traceback.format_exc()}")
            return None
    
    def calculate_cvar(self, returns, confidence_level=0.05):
        """Calculate Conditional Value at Risk (Expected Shortfall)"""
        try:
            if returns is None or returns.empty:
                print("ERROR: No returns data provided for CVaR calculation")
                return None
            
            var = self.calculate_var(returns, confidence_level)
            if var is None:
                return None
            
            return returns[returns <= var].mean()
        except Exception as e:
            print(f"ERROR: Failed to calculate CVaR: {e}")
            logging.error(f"CVaR calculation error: {traceback.format_exc()}")
            return None
    
    def technical_analysis(self, prices):
        """Perform technical analysis"""
        try:
            if prices is None or prices.empty:
                print("ERROR: No price data provided for technical analysis")
                return {}
            
            # Handle both DataFrame and Series inputs
            if hasattr(prices, 'columns'):
                # DataFrame input
                if 'Close' not in prices.columns:
                    print("ERROR: 'Close' column not found in price data")
                    return {}
                price_series = prices['Close']
            else:
                # Series input - assume it's already the price series
                price_series = prices
            
            # Simple Moving Averages
            sma_20 = price_series.rolling(window=20).mean()
            sma_50 = price_series.rolling(window=50).mean()
            sma_200 = price_series.rolling(window=200).mean()
            
            # RSI
            delta = price_series.diff()
            gain = (delta.where(delta > 0, 0)).rolling(window=14).mean()
            loss = (-delta.where(delta < 0, 0)).rolling(window=14).mean()
            rs = gain / loss
            rsi = 100 - (100 / (1 + rs))
            
            # MACD
            ema_12 = price_series.ewm(span=12).mean()
            ema_26 = price_series.ewm(span=26).mean()
            macd = ema_12 - ema_26
            macd_signal = macd.ewm(span=9).mean()
            macd_histogram = macd - macd_signal
            
            # Bollinger Bands
            bb_middle = price_series.rolling(window=20).mean()
            bb_std = price_series.rolling(window=20).std()
            bb_upper = bb_middle + (bb_std * 2)
            bb_lower = bb_middle - (bb_std * 2)
            
            return {
                'SMA_20': sma_20,
                'SMA_50': sma_50,
                'SMA_200': sma_200,
                'RSI': rsi,
                'MACD': macd,
                'MACD_Signal': macd_signal,
                'MACD_Histogram': macd_histogram,
                'BB_Upper': bb_upper,
                'BB_Middle': bb_middle,
                'BB_Lower': bb_lower
            }
        except Exception as e:
            print(f"ERROR: Failed to perform technical analysis: {e}")
            logging.error(f"Technical analysis error: {traceback.format_exc()}")
            return {}
    
    def fundamental_analysis(self, ticker):
        """Perform fundamental analysis using available data"""
        try:
            if not ticker:
                print("ERROR: No ticker provided for fundamental analysis")
                return {}
            
            stock = yf.Ticker(ticker)
            info = stock.info
            
            if not info:
                print(f"WARNING: No fundamental data available for {ticker}")
                return {}
            
            # Extract key metrics
            metrics = {
                'PE_Ratio': info.get('trailingPE'),
                'PB_Ratio': info.get('priceToBook'),
                'PS_Ratio': info.get('priceToSalesTrailing12Months'),
                'Debt_to_Equity': info.get('debtToEquity'),
                'Current_Ratio': info.get('currentRatio'),
                'ROE': info.get('returnOnEquity'),
                'ROA': info.get('returnOnAssets'),
                'Profit_Margin': info.get('profitMargins'),
                'Revenue_Growth': info.get('revenueGrowth'),
                'Earnings_Growth': info.get('earningsGrowth'),
                'Dividend_Yield': info.get('dividendYield'),
                'Payout_Ratio': info.get('payoutRatio')
            }
            
            return metrics
        except Exception as e:
            print(f"ERROR: Failed to fetch fundamental data for {ticker}: {e}")
            logging.error(f"Fundamental analysis error for {ticker}: {traceback.format_exc()}")
            return {}
    
    def risk_analysis(self, returns, benchmark_returns=None):
        """Perform comprehensive risk analysis"""
        try:
            if returns is None or returns.empty:
                print("ERROR: No returns data provided for risk analysis")
                return {}
            
            # Basic risk metrics
            volatility = returns.std() * np.sqrt(252)
            annual_return = returns.mean() * 252
            
            # Handle division by zero
            if volatility > 0:
                sharpe_ratio = (annual_return - RISK_FREE_RATE) / volatility
            else:
                sharpe_ratio = 0
                print("WARNING: Zero volatility detected, setting Sharpe ratio to 0")
            
            # Maximum drawdown
            cumulative_returns = (1 + returns).cumprod()
            max_drawdown = self.calculate_max_drawdown(cumulative_returns)
            
            # Value at Risk
            var_95 = self.calculate_var(returns, 0.05)
            cvar_95 = self.calculate_cvar(returns, 0.05)
            
            # Beta calculation if benchmark provided
            beta = None
            if benchmark_returns is not None and not benchmark_returns.empty:
                try:
                    benchmark_var = benchmark_returns.var()
                    if benchmark_var > 0:
                        beta = returns.cov(benchmark_returns) / benchmark_var
                    else:
                        print("WARNING: Zero benchmark variance, cannot calculate beta")
                except Exception as e:
                    print(f"ERROR: Failed to calculate beta: {e}")
            
            return {
                'Volatility': volatility,
                'Annual_Return': annual_return,
                'Sharpe_Ratio': sharpe_ratio,
                'Max_Drawdown': max_drawdown,
                'VaR_95': var_95,
                'CVaR_95': cvar_95,
                'Beta': beta
            }
        except Exception as e:
            print(f"ERROR: Failed to perform risk analysis: {e}")
            logging.error(f"Risk analysis error: {traceback.format_exc()}")
            return {}
    
    def correlation_analysis(self, returns_data):
        """Analyze correlations between stocks"""
        try:
            if returns_data is None or returns_data.empty:
                print("ERROR: No returns data provided for correlation analysis")
                return None
            
            correlation_matrix = returns_data.corr()
            return correlation_matrix
        except Exception as e:
            print(f"ERROR: Failed to perform correlation analysis: {e}")
            logging.error(f"Correlation analysis error: {traceback.format_exc()}")
            return None
    
    def portfolio_optimization(self, returns_data, method='sharpe'):
        """Optimize portfolio weights using different methods"""
        try:
            if returns_data is None or returns_data.empty:
                print("ERROR: No returns data provided for portfolio optimization")
                return None
            
            n_assets = len(returns_data.columns)
            if n_assets == 0:
                print("ERROR: No assets found in returns data")
                return None
            
            # Calculate expected returns and covariance matrix
            expected_returns = returns_data.mean() * 252
            cov_matrix = returns_data.cov() * 252
            
            # Check for valid covariance matrix
            if cov_matrix.isnull().any().any():
                print("ERROR: Invalid covariance matrix (contains NaN values)")
                return None
            
            if method == 'sharpe':
                # Maximize Sharpe ratio
                def objective(weights):
                    try:
                        portfolio_return = np.sum(expected_returns * weights)
                        portfolio_vol = np.sqrt(np.dot(weights.T, np.dot(cov_matrix, weights)))
                        if portfolio_vol > 0:
                            sharpe = (portfolio_return - RISK_FREE_RATE) / portfolio_vol
                        else:
                            sharpe = 0
                        return -sharpe  # Minimize negative Sharpe ratio
                    except Exception as e:
                        print(f"ERROR: Objective function failed: {e}")
                        return 1e6  # Return large value to avoid this solution
                
            elif method == 'min_variance':
                # Minimize variance
                def objective(weights):
                    try:
                        portfolio_vol = np.sqrt(np.dot(weights.T, np.dot(cov_matrix, weights)))
                        return portfolio_vol
                    except Exception as e:
                        print(f"ERROR: Objective function failed: {e}")
                        return 1e6
                
            elif method == 'max_return':
                # Maximize return
                def objective(weights):
                    try:
                        portfolio_return = np.sum(expected_returns * weights)
                        return -portfolio_return  # Minimize negative return
                    except Exception as e:
                        print(f"ERROR: Objective function failed: {e}")
                        return 1e6
            else:
                print(f"ERROR: Unknown optimization method: {method}")
                return None
            
            # Constraints
            constraints = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1})  # Weights sum to 1
            bounds = tuple((0, 1) for _ in range(n_assets))  # Weights between 0 and 1
            
            # Initial guess (equal weights)
            initial_weights = np.array([1/n_assets] * n_assets)
            
            # Optimize
            try:
                result = minimize(objective, initial_weights, method='SLSQP', 
                                bounds=bounds, constraints=constraints)
                
                if result.success:
                    optimal_weights = result.x
                    portfolio_return = np.sum(expected_returns * optimal_weights)
                    portfolio_vol = np.sqrt(np.dot(optimal_weights.T, np.dot(cov_matrix, optimal_weights)))
                    
                    if portfolio_vol > 0:
                        sharpe_ratio = (portfolio_return - RISK_FREE_RATE) / portfolio_vol
                    else:
                        sharpe_ratio = 0
                    
                    return {
                        'weights': optimal_weights,
                        'expected_return': portfolio_return,
                        'volatility': portfolio_vol,
                        'sharpe_ratio': sharpe_ratio
                    }
                else:
                    print(f"ERROR: Portfolio optimization failed: {result.message}")
                    return None
            except Exception as e:
                print(f"ERROR: Portfolio optimization failed: {e}")
                logging.error(f"Portfolio optimization error: {traceback.format_exc()}")
                return None
                
        except Exception as e:
            print(f"ERROR: Failed to perform portfolio optimization: {e}")
            logging.error(f"Portfolio optimization error: {traceback.format_exc()}")
            return None
    
    def backtest_portfolio(self, returns_data, weights, initial_investment=100000):
        """Backtest portfolio performance"""
        try:
            if returns_data is None or returns_data.empty:
                print("ERROR: No returns data provided for backtesting")
                return None
            
            if weights is None or len(weights) == 0:
                print("ERROR: No weights provided for backtesting")
                return None
            
            if len(weights) != len(returns_data.columns):
                print(f"ERROR: Weight count ({len(weights)}) doesn't match asset count ({len(returns_data.columns)})")
                return None
            
            # Calculate portfolio returns
            portfolio_returns = returns_data.dot(weights)
            
            # Calculate portfolio value over time
            portfolio_values = initial_investment * (1 + portfolio_returns).cumprod()
            
            # Calculate performance metrics
            total_return = (portfolio_values.iloc[-1] - initial_investment) / initial_investment
            annual_return = portfolio_returns.mean() * 252
            volatility = portfolio_returns.std() * np.sqrt(252)
            
            if volatility > 0:
                sharpe_ratio = (annual_return - RISK_FREE_RATE) / volatility
            else:
                sharpe_ratio = 0
            
            max_drawdown = self.calculate_max_drawdown(portfolio_values / initial_investment)
            
            return {
                'portfolio_values': portfolio_values,
                'portfolio_returns': portfolio_returns,
                'total_return': total_return,
                'annual_return': annual_return,
                'volatility': volatility,
                'sharpe_ratio': sharpe_ratio,
                'max_drawdown': max_drawdown
            }
        except Exception as e:
            print(f"ERROR: Failed to backtest portfolio: {e}")
            logging.error(f"Portfolio backtest error: {traceback.format_exc()}")
            return None
    
    def generate_analysis_report(self, ticker, start_date=None, end_date=None):
        """Generate comprehensive analysis report for a company"""
        try:
            if not ticker:
                print("ERROR: No ticker provided for analysis report")
                return None
            
            report_lines = []
            report_lines.append(f"\n{'='*60}")
            report_lines.append(f"COMPREHENSIVE ANALYSIS REPORT: {ticker}")
            report_lines.append(f"{'='*60}")
            
            # Get stock data - try database first, fallback to yfinance
            prices = None
            try:
                if self.db_manager.connect():
                    prices = self.db_manager.get_stock_prices(ticker, start_date, end_date)
                    self.db_manager.disconnect()
            except Exception as e:
                print(f"WARNING: Database connection failed, using yfinance: {e}")
                logging.warning(f"Database connection failed for {ticker}: {e}")
            
            if prices is None or prices.empty:
                # Fallback to yfinance
                try:
                    print(f"Fetching data for {ticker} from yfinance...")
                    stock = yf.Ticker(ticker)
                    prices = stock.history(start=start_date or DATA_START_DATE, end=end_date or DATA_END_DATE)
                    
                    if prices.empty:
                        print(f"WARNING: No data available for {ticker} from yfinance")
                        return None
                        
                except Exception as e:
                    print(f"ERROR: Failed to fetch data for {ticker}: {e}")
                    logging.error(f"Data fetching error for {ticker}: {traceback.format_exc()}")
                    return None
            
            # Calculate returns
            returns, cumulative_returns = self.calculate_returns(prices)
            if returns is None:
                print(f"ERROR: Failed to calculate returns for {ticker}")
                return None
            
            # 1. Technical Analysis
            report_lines.append("\n1. TECHNICAL ANALYSIS")
            report_lines.append("-" * 30)
            technical = self.technical_analysis(prices)
            
            if not technical:
                report_lines.append("ERROR: Technical analysis failed")
                return None
            
            try:
                # Get current price - handle both DataFrame and Series
                if hasattr(prices, 'columns'):
                    current_price = prices['Close'].iloc[-1]
                else:
                    current_price = prices.iloc[-1]
                
                sma_20_current = technical['SMA_20'].iloc[-1]
                sma_50_current = technical['SMA_50'].iloc[-1]
                rsi_current = technical['RSI'].iloc[-1]
                
                report_lines.append(f"Current Price: ${current_price:.2f}")
                report_lines.append(f"20-day SMA: ${sma_20_current:.2f}")
                report_lines.append(f"50-day SMA: ${sma_50_current:.2f}")
                report_lines.append(f"RSI: {rsi_current:.2f}")
                
                # Technical signals
                if current_price > sma_20_current > sma_50_current:
                    report_lines.append("Technical Signal: BULLISH (Price above both SMAs)")
                elif current_price < sma_20_current < sma_50_current:
                    report_lines.append("Technical Signal: BEARISH (Price below both SMAs)")
                else:
                    report_lines.append("Technical Signal: NEUTRAL")
                
                if rsi_current > 70:
                    report_lines.append("RSI Signal: OVERBOUGHT")
                elif rsi_current < 30:
                    report_lines.append("RSI Signal: OVERSOLD")
                else:
                    report_lines.append("RSI Signal: NEUTRAL")
            except Exception as e:
                print(f"ERROR: Failed to process technical indicators for {ticker}: {e}")
                report_lines.append("ERROR: Failed to process technical indicators")
            
            # 2. Risk Analysis
            report_lines.append("\n2. RISK ANALYSIS")
            report_lines.append("-" * 30)
            risk_metrics = self.risk_analysis(returns)
            
            if risk_metrics:
                for metric, value in risk_metrics.items():
                    if value is not None:
                        try:
                            if 'Ratio' in metric or 'Return' in metric:
                                report_lines.append(f"{metric}: {value:.4f}")
                            elif 'Drawdown' in metric:
                                report_lines.append(f"{metric}: {value:.2%}")
                            else:
                                report_lines.append(f"{metric}: {value:.6f}")
                        except Exception as e:
                            print(f"ERROR: Failed to format metric {metric}: {e}")
                            report_lines.append(f"{metric}: {value}")
            else:
                report_lines.append("ERROR: Risk analysis failed")
            
            # 3. Fundamental Analysis
            report_lines.append("\n3. FUNDAMENTAL ANALYSIS")
            report_lines.append("-" * 30)
            fundamental = self.fundamental_analysis(ticker)
            
            if fundamental:
                for metric, value in fundamental.items():
                    if value is not None:
                        try:
                            if 'Yield' in metric or 'Growth' in metric or 'Ratio' in metric:
                                report_lines.append(f"{metric}: {value:.4f}")
                            else:
                                report_lines.append(f"{metric}: {value:.2f}")
                        except Exception as e:
                            print(f"ERROR: Failed to format fundamental metric {metric}: {e}")
                            report_lines.append(f"{metric}: {value}")
            else:
                report_lines.append("WARNING: No fundamental data available")
            
            # 4. Investment Recommendation
            report_lines.append("\n4. INVESTMENT RECOMMENDATION")
            report_lines.append("-" * 30)
            
            score = 0
            reasons = []
            
            try:
                # Technical score
                if current_price > sma_20_current > sma_50_current:
                    score += 2
                    reasons.append("Strong technical trend")
                elif current_price < sma_20_current < sma_50_current:
                    score -= 1
                    reasons.append("Weak technical trend")
                
                if 30 <= rsi_current <= 70:
                    score += 1
                    reasons.append("RSI in normal range")
                elif rsi_current < 30:
                    score += 1
                    reasons.append("Oversold conditions")
                elif rsi_current > 70:
                    score -= 1
                    reasons.append("Overbought conditions")
                
                # Risk score
                if risk_metrics.get('Sharpe_Ratio') and risk_metrics['Sharpe_Ratio'] > 1.0:
                    score += 2
                    reasons.append("Strong risk-adjusted returns")
                elif risk_metrics.get('Sharpe_Ratio') and risk_metrics['Sharpe_Ratio'] > 0.5:
                    score += 1
                    reasons.append("Good risk-adjusted returns")
                elif risk_metrics.get('Sharpe_Ratio') and risk_metrics['Sharpe_Ratio'] < 0:
                    score -= 1
                    reasons.append("Poor risk-adjusted returns")
                
                if risk_metrics.get('Max_Drawdown') and risk_metrics['Max_Drawdown'] > -0.2:
                    score += 1
                    reasons.append("Acceptable drawdown")
                elif risk_metrics.get('Max_Drawdown'):
                    score -= 1
                    reasons.append("High drawdown risk")
                
                # Fundamental score
                if fundamental.get('PE_Ratio') and fundamental['PE_Ratio'] < PE_RATIO_THRESHOLD:
                    score += 1
                    reasons.append("Attractive P/E ratio")
                
                if fundamental.get('ROE') and fundamental['ROE'] > ROE_THRESHOLD:
                    score += 1
                    reasons.append("Strong ROE")
                
                # Generate recommendation
                if score >= 4:
                    recommendation = "STRONG BUY"
                elif score >= 2:
                    recommendation = "BUY"
                elif score >= 0:
                    recommendation = "HOLD"
                elif score >= -2:
                    recommendation = "SELL"
                else:
                    recommendation = "STRONG SELL"
                
                report_lines.append(f"Overall Score: {score}")
                report_lines.append(f"Recommendation: {recommendation}")
                report_lines.append("Key Factors:")
                for reason in reasons:
                    report_lines.append(f"  • {reason}")
                    
            except Exception as e:
                print(f"ERROR: Failed to generate investment recommendation for {ticker}: {e}")
                report_lines.append("ERROR: Failed to generate investment recommendation")
                recommendation = "ERROR"
                score = 0
                reasons = []
            
            return {
                'ticker': ticker,
                'score': score,
                'recommendation': recommendation,
                'reasons': reasons,
                'technical': technical,
                'risk_metrics': risk_metrics,
                'fundamental': fundamental,
                'report_text': '\n'.join(report_lines)
            }
            
        except Exception as e:
            print(f"ERROR: Failed to generate analysis report for {ticker}: {e}")
            logging.error(f"Analysis report error for {ticker}: {traceback.format_exc()}")
            return None
    
    def analyze_all_companies(self):
        """Analyze all companies and generate comparative report"""
        try:
            results = []
            detailed_reports = []
            
            print(f"Starting analysis of {len(self.companies)} companies...")
            
            for name, ticker in self.companies.items():
                try:
                    print(f"\nAnalyzing {name} ({ticker})...")
                    result = self.generate_analysis_report(ticker)
                    if result:
                        results.append(result)
                        if 'report_text' in result:
                            detailed_reports.append(result['report_text'])
                        print(f"✓ Completed analysis for {name}")
                    else:
                        print(f"✗ Failed to analyze {name}")
                except Exception as e:
                    print(f"ERROR: Failed to analyze {name} ({ticker}): {e}")
                    logging.error(f"Company analysis error for {name} ({ticker}): {traceback.format_exc()}")
                    continue
            
            print(f"\nAnalysis completed. Successfully analyzed {len(results)} out of {len(self.companies)} companies.")
            return results, detailed_reports
            
        except Exception as e:
            print(f"ERROR: Failed to analyze all companies: {e}")
            logging.error(f"Analyze all companies error: {traceback.format_exc()}")
            return [], []

if __name__ == "__main__":
    try:
        print("Starting Investment Analysis...")
        analyzer = InvestmentAnalyzer()
        results, reports = analyzer.analyze_all_companies()
        
        if results:
            print(f"\nAnalysis Summary:")
            print(f"Successfully analyzed {len(results)} companies")
            
            # Print summary of results
            for result in results:
                ticker = result.get('ticker', 'Unknown')
                recommendation = result.get('recommendation', 'Unknown')
                score = result.get('score', 0)
                print(f"{ticker}: {recommendation} (Score: {score})")
        else:
            print("No analysis results generated")
            
    except Exception as e:
        print(f"CRITICAL ERROR: Failed to run investment analysis: {e}")
        logging.critical(f"Main execution error: {traceback.format_exc()}")
        sys.exit(1) 