...
Zero duplication, comprehensive analysis with CSV export
"""
...
def check_prerequisites():
    ...
    config_exists = os.path.exists("scripts/config.py")
    
    if not config_exists:
        print("❌ config.py not found.")
        print("Please run setup.py first: python3 scripts/setup.py")
        return False
...
try:
    # Try importing from scripts directory first (when run from root)
    from scripts.config import *
    from scripts.database import DatabaseManager
except ImportError:
    try:
        # Try importing directly (when run from scripts directory)
        from config import *
        from database import DatabaseManager
    except ImportError as e:
        print(f"ERROR: Failed to import config or database modules: {e}")
        print("Please ensure config.py and database.py are in the scripts directory")
        sys.exit(1)

class UnifiedInvestmentPipeline:
    """Unified pipeline with zero duplication and comprehensive analysis"""
    ...
    def export_raw_data(self):
        """Export raw data to data directory"""
        print("\n📊 Exporting raw data to data directory...")
        
        try:
            # Create data directory if it doesn't exist
            os.makedirs('data', exist_ok=True)
            
            # Export raw stock prices
            for name, ticker in self.companies.items():
                if ticker in self.stock_prices:
                    raw_prices = self.stock_prices[ticker].copy()
                    raw_prices.reset_index(inplace=True)
                    
                    # Save raw price data
                    output_path = f'data/{ticker}_raw_prices.csv'
                    raw_prices.to_csv(output_path, index=False)
                    print(f"✅ Raw prices exported for {ticker} to {output_path}")
            
            print("✅ Raw data export completed")
            return True
            
        except Exception as e:
            print(f"❌ ERROR: Failed to export raw data: {e}")
            logger.error(f"Raw data export error: {e}")
            return False

    def export_analysis_data(self):
        """Step 5: Export all analysis results to CSV files"""
        print("\n📊 Step 5: Exporting all analysis results to CSV files...")
        ...
        try:
            ...
            if self.company_summaries:
                ...
                output_path = 'results/company_summary.csv'
            ...
            if self.stock_prices:
                ...
                if all_prices:
                    ...
                    output_path = 'results/stock_prices.csv'
            ...
            if self.risk_metrics:
                ...
                output_path = 'results/risk_metrics.csv'
            ...
            if not self.technical_indicators.empty:
                ...
                output_path = 'results/technical_indicators.csv'
            ...
            print(f"\n✅ Analysis data export completed successfully!")
            print(f"📁 Exported {len(exported_files)} files to 'results/' directory:")
            ...
            print(f"❌ ERROR: Failed to export analysis data: {e}")
            logger.error(f"Analysis data export error: {e}")
            return False
    ...
    def generate_markdown_report(self):
        ...
        try:
            ...
            report_filename = f"results/investment_analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    def run_complete_pipeline(self):
        ...
        try:
            ...
            # Step 4: Export raw data to data/raw directory
            self.export_raw_data()
            
            # Step 5: Export all results to CSV files
            if not self.export_analysis_data():
                print("❌ ERROR: Failed to export analysis data.")
                return False
    ...
    def generate_summary(self):
        ...
        try:
            ...
            print(f"  • data/ - Raw stock price data for each company")
            print(f"  • results/ - CSV files and reports with all analysis results")
            ...
            print(f"📊 Analysis data ready in 'results/' directory")
...
def main():
    ...
    print("=" * 80)
    print("🎯 Zero duplication • Comprehensive analysis • CSV export")
    print("=" * 80)
    ...
    try:
        ...
        if success:
            ...
            print("• 'data/' - Raw stock price data for each company")
            print("• 'results/' - Complete analysis dataset and reports")
            ...
            print("  - investment_analysis_report_YYYYMMDD_HHMMSS.md - Comprehensive analysis report")
            print("\n📈 Next Steps:")
            print("1. Review the CSV files in 'results/' directory")
            print("2. Use any data analysis tool (Excel, Python, R, etc.)")
            print("3. Review the markdown report in 'results/' directory")
            print("4. Create your own visualizations and dashboards!")
